...
Output is bulky, and we can turn off Python expressions and see how this internal data structure can be rendered neatly
...
Our toy decompiler can indetify patterns like such and rewrite them.
...
\dots but in fact, it's all \TT{reduced by reduce\_AND2()} function we already saw (\ref{AND2}):
...
But is it correct?
I've added a function which can output expression(s) to SMTLIB-format, it's as simple as a function which converts
expression to string.

And this is SMTLIB-file to be Z3 solver:
...
\TT{bind\_expr()} takes pointer to a pointer, which will points to the part of expressions, again, in case of success.
